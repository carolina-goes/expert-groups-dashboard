...
def fetch_all_extras(records, max_workers=16):
    """Fetch members + statistics + additional info for all groups (parallel)."""
    extras_map = {}
    ids = [rec.get("_numericId") for rec in records if rec.get("_numericId") is not None]
    total = len(ids)
    done = 0

    def _safe_fetch(gid):
        try:
            return gid, fetch_all_extras_for_group(gid)
        except Exception:
            return gid, {"members": {}, "countries": [], "statistics": {}, "additionalInfo": {}}

    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        futures = [ex.submit(_safe_fetch, gid) for gid in ids]
        for fut in as_completed(futures):
            ...
            if done % 20 == 0 or done == total:
                pct = 100 * done // total
                print(f"\r  Progress (extras): {done}/{total} ({pct}%)", end="", flush=True)
    print()
    return extras_map


def compact_member(m, cat):
    """Compact a single member record â captures ALL available fields."""
    base = {
